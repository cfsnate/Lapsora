...
- [ ] **REC-01**: User can enable continuous 24/7 recording on a per-profile basis using FFmpeg segmented MPEG-TS output
- [x] **REC-02**: User can configure recording settings per profile (segment duration, storage location, enable/disable)
- [ ] **REC-03**: Recording automatically recovers from stream disconnects and resumes when the stream is available
